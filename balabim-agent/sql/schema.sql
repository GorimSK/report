...
-- __DS__ nahradí build.py hodnotou `projekt.dataset` zo settings.json.
...
CREATE TABLE IF NOT EXISTS `__DS__.daily_perf` (
  date DATE NOT NULL,
...
CREATE TABLE IF NOT EXISTS `__DS__.search_terms` (
...
CREATE TABLE IF NOT EXISTS `__DS__.meta_adset_7d` (
  snapshot_date DATE NOT NULL,
  account_id STRING,
  country STRING,
  campaign_id STRING,
...
CREATE TABLE IF NOT EXISTS `__DS__.entity_issues` (
  snapshot_date DATE NOT NULL,
  platform STRING,
  account_id STRING,
  country STRING,
...
CREATE TABLE IF NOT EXISTS `__DS__.fx_rates` (
  date DATE NOT NULL,
...
CREATE TABLE IF NOT EXISTS `__DS__.findings` (
...
CREATE TABLE IF NOT EXISTS `__DS__.agent_tasks` (
...
CREATE TABLE IF NOT EXISTS `__DS__.actions_log` (
  task_gid STRING NOT NULL,
...
CREATE TABLE IF NOT EXISTS `__DS__.action_outcomes` (
  task_gid STRING NOT NULL,
  eval_day INT64,                      -- 7 | 14
...
CREATE OR REPLACE VIEW `__DS__.v_daily_perf` AS
SELECT * EXCEPT (rn)
FROM (
  SELECT *,
    ROW_NUMBER() OVER (
